/**
 * This class is holding 3 functions
...
public class WindsorOpenDataApi {
	...
	/**
	 * This API will return waste collection schedule
	 * for upcoming 1 week in Windsor
	 * 
	 * @return
	 */
	public String[] getGarabageCollDetails(){
		...
		try {
			url = new URL(
					"http://opendata.citywindsor.ca/api/events/wasteCollection?start="+ 
					startDate+ "&end="+ endDate +"&location=1A");
			...
			try {
				...
				for (int i = 0; i < array3.size(); i++) {
					...
					scheduleList[i] = title.concat(array4.get("title").toString()).concat(date).concat(array4.get("end").toString().substring(0, 10));
				}
	...
	public String[] getConstructionDetails(){
		...
		try {
			url = new URL(
					"http://opendata.citywindsor.ca/api/construction?start="+ 
			...
			try {
				...
				for (int i = 0; i < array.size(); i++) {
					...
					if(d3.compareTo(d2) <= 0 && (d4.compareTo(d1) >=0)
						...
						scheduleList.add(date1.concat(array2.get("start").toString().substring(0, 10))
								.concat(date2).concat(array2.get("end").toString().substring(0, 10))
								.concat(place1).concat(array2.get("fromStreet").toString())
								.concat(place2).concat(array2.get("toStreet").toString())
	...
	/**
	 * This API will return public events 
	 * for upcoming 1 week in Windsor
	 * 
	 * @return
	 */
	public String[] getEventDetails(){
		...
		try {
			url = new URL(
					"http://opendata.citywindsor.ca/api/events/snapd?start="+ 
					startDate+ "&end="+ endDate);
			System.out.println("URL used :" + url );
			conn = (HttpURLConnection) url.openConnection();
			conn.setRequestMethod("GET");
			...
			try {
				...
				for (int i = 0; i < array3.size(); i++) {
					...
					scheduleList[i] = title.concat(array4.get("title").toString())
							.concat(date).concat(array4.get("start").toString().substring(0, 10))
							.concat(date1).concat(array4.get("end").toString().substring(0, 10))
							.concat(location).concat(array4.get("googleaddress").toString())
							.concat(infoLink).concat(array4.get("link").toString())
